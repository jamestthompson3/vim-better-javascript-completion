...
if exists('g:vimjs#reactapis') && g:vimjs#reactapis == 1
  let s:react = ['Component {', 'PureComponent {', 'memo(', 'createElement(', 'createFactory(', 'cloneElement(', 'isValidElement(', 'Children', 'Fragment', 'createRef()', 'forwardRef()', 'lazy', 'Suspense']
  let s:reactchildren = [ 'map(', 'forEach(', 'count(', 'only(', 'toArray(' ]
  let s:styledcomponents = [ 'title', 'body', 'section', 'nav', 'article', 'aside', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'hgroup', 'header', 'footer', 'address', 'p', 'hr', 'pre', 'blockquote', 'ol', 'ul', 'li', 'dl', 'dt', 'dd', 'figure', 'figcaption', 'div', 'a', 'em', 'strong', 'small', 's', 'cite', 'q cite', 'dfn', 'abbr', 'data', 'time', 'code', 'var', 'samp', 'kbd', 'sub', 'sup', 'i', 'b', 'u', 'mark', 'rt', 'rp', 'bdi', 'bdo', 'span', 'br', 'wbr', 'ins', 'del', 'img', 'iframe', 'embed', 'video', 'audio', 'source', 'track', 'canvas', 'map', 'area', 'table', 'caption', 'colgroup', 'col', 'tbody', 'thead', 'tfoot', 'tr', 'td', 'th', 'form', 'fieldset', 'legend', 'label', 'input', 'button', 'select', 'datalist', 'optgroup', 'option', 'textarea', 'keygen', 'output', 'progress', 'meter', 'details', 'summary', 'command', 'menu', 'dialog']
else
  let g:vimjs#reactapis = 0
endif
...
function! js#CompleteJS(findstart, base)
  if a:findstart
    ...
    if shortcontext =~ '\.$'
      ...
      if !exists('values') && g:vimjs#reactapis == 1
        if shortcontext =~ 'React\.$'
          let values = s:react
        elseif shortcontext =~ 'React.Children\.'
          let values = s:reactchildren
        elseif shortcontext =~ 'styled.$'
          let values = s:styledcomponents
        elseif shortcontext =~ 'style={{\s*[a-zA-Z]?\+}}'
          let values = s:styls
        endif
      endif
